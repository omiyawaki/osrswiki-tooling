...
validate_root_directory() {
    ...
    if [[ ${#root_additions[@]} -gt 0 ]] || [[ ${#root_directories[@]} -gt 0 ]] || [[ ${#platform_contamination[@]} -gt 0 ]]; then
        ...
        echo
        echo "2. 🗑️ DELETE: If temporary debugging, testing, or no longer needed"
        echo "   → Examples: debug logs, test scripts, screenshots, analysis dirs"
        echo
        echo "3. 💡 COMMON PATTERNS:"
        echo "   → Debug/analysis files → DELETE (*.md analysis docs, debug dirs)"
        echo "   → Test scripts → DELETE or move to tools/testing/"
        echo "   → Screenshots → DELETE or move to screenshots/"
