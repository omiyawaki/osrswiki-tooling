...
echo -e "${YELLOW}📋 Phase 6: Backup Infrastructure Health${NC}"
echo "-------------------------------------"

if [[ -d "$HOME/Backups/osrswiki" ]]; then
    health_success "Backup directory exists at ~/Backups/osrswiki"
    
    # Check for recent backups
    RECENT_BACKUPS=$(find "$HOME/Backups/osrswiki" -name "*.bundle" -mtime -7 2>/dev/null | wc -l)
    if [[ "$RECENT_BACKUPS" -gt 0 ]]; then
        health_success "Found $RECENT_BACKUPS recent backup files (last 7 days)"
    else
        health_warning "No recent backup files found (consider running emergency-backup.sh)"
    fi
else
    health_warning "Backup directory not found at ~/Backups/osrswiki"
fi

# Check git hooks
echo -e "${YELLOW}📋 Phase 7: Safety Infrastructure Health${NC}"
echo "-------------------------------------"
