...
for file in "$MONOREPO_ROOT"/{README.md,CLAUDE.md,.gitignore,.editorconfig}; do
    if [[ -f "$file" ]]; then
...
# Copy any dotfiles that might be important (excluding .git and common temp files)
for file in "$MONOREPO_ROOT"/.*; do
    if [[ -f "$file" ]]; then
        basename_file=$(basename "$file")
        if [[ "$basename_file" != ".git" && "$basename_file" != ".DS_Store" && "$basename_file" != ".." && "$basename_file" != "." ]]; then
